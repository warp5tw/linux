...
/* GPIO registers */
#define NPCM8XX_GP_N_TLOCK1	0x0
#define NPCM8XX_GP_N_DIN	0x4 /* Data IN */
#define NPCM8XX_GP_N_POL	0x8 /* Polarity */
#define NPCM8XX_GP_N_DOUT	0xC /* Data OUT */
...
static const struct npcm8xx_pincfg pincfg[] = {
	...
	NPCM8XX_PINCFG(169,	scipme, MFSEL3, 0,	smb21, MFSEL4, 29,	none, NONE, 0,		none, NONE, 0,		none, NONE, 0,		0),
	NPCM8XX_PINCFG(170,	sci, MFSEL1, 22,	smb21, MFSEL4, 29,	none, NONE, 0,		none, NONE, 0,		none, NONE, 0,		0),
...
//	NPCM8XX_PINCFG(226,	spix, MFSEL4, 27,	none, NONE, 0,		none, NONE, 0,		none, NONE, 0,		none, NONE, 0,		DS(8, 12) | SLEW),
...
static const struct pinctrl_pin_desc npcm8xx_pins[] = {
	...
	PINCTRL_PIN(33, "I3C4_SCL"),
	PINCTRL_PIN(34, "I3C4_SDA"),
			    ...
			    int pin_number, int mode)
{
	...
	for (i = 0 ; i < pin_number ; i++) {
		cfg = &pincfg[pin[i]];
		if (mode == fn_gpio || cfg->fn0 == mode || cfg->fn1 == mode || cfg->fn2 == mode || cfg->fn3 == mode || cfg->fn4 == mode) {
			if (cfg->reg0)
				 ...
				 struct regmap *gcr_regmap, unsigned int pin)
{
	...
	if (pincfg[pin].flag & SLEW)
		return ioread32(bank->base + NPCM8XX_GP_N_OSRC)
		& pinmask;
...
static int npcm8xx_gpio_reset_persist(struct npcm8xx_pinctrl *npcm,
					unsigned int pin, int enable)
{
	...
	if (enable) {
		regmap_update_bits(npcm->rst_regmap, NPCM8XX_RST_WD0RCR,
				   GPIOX_MODULE_RESET << banknum, 0);
		regmap_update_bits(npcm->rst_regmap, NPCM8XX_RST_WD1RCR,
				   GPIOX_MODULE_RESET << banknum, 0);
		regmap_update_bits(npcm->rst_regmap, NPCM8XX_RST_WD2RCR,
				   GPIOX_MODULE_RESET << banknum, 0);
		regmap_update_bits(npcm->rst_regmap, NPCM8XX_RST_CORSTC,
				   GPIOX_MODULE_RESET << banknum, 0);
	} else {
		regmap_update_bits(npcm->rst_regmap, NPCM8XX_RST_WD0RCR,
...
static int npcm8xx_get_group_pins(struct pinctrl_dev *pctldev,
				  ...
				  unsigned int *npins)
{
	*npins = npcm8xx_groups[selector].npins;
	*pins  = npcm8xx_groups[selector].pins;
...
static struct pinctrl_ops npcm8xx_pinctrl_ops = {
	...
	.pin_dbg_show = npcm8xx_pin_dbg_show,
				  ...
				  unsigned int group)
{
	...
	npcm8xx_setfunc(npcm->gcr_regmap, npcm8xx_groups[group].pins,
...
static struct pinmux_ops npcm8xx_pinmux_ops = {
				  ...
				  unsigned int pin, unsigned long config)
{
	...
	case PIN_CONFIG_PERSIST_STATE:
		return npcm8xx_gpio_reset_persist(npcm, pin, arg);
	default:
		return -ENOTSUPP;
	}
...
static struct pinconf_ops npcm8xx_pinconf_ops = {
	.is_generic = true,
...
static int npcm8xx_pinctrl_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pctrl->rst_regmap)) {
		dev_err(pctrl->dev, "didn't find nuvoton,npcm845-rst\n");
